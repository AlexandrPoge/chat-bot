...
export function OverviewPage(props: Props) {
  const { activeDocument, documents, settings, stats, messages, isAnswering, published, userName, onAsk, onGoTo } = props;
  ...
  const metrics: [string, string, string, string, LucideIcon, number][] = [
    ["Knowledge sources", String(stats.sources), stats.sources ? "Synced in Supabase" : "Add a cloud source", "Every ready source is stored privately and indexed for this bot.", Database, Math.min(100, stats.sources * 5)],
    ["Answers", String(stats.answers), `${stats.conversations} conversations`, "Live customer and Gemini preview conversations are persisted in Supabase.", MessageCircleMore, Math.min(100, stats.answers)],
    ["Grounded answers", stats.answers ? `${confidence}%` : "—", stats.answers ? "Measured from sources" : "No live answers yet", "This ratio is calculated from assistant messages linked to a source document.", BadgeCheck, confidence],
  ];
  const [selectedMetric, setSelectedMetric] = useState(0);
  const [, value, , detail] = metrics[selectedMetric];
  return <div className="space-y-5">
    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-sm font-medium text-[#73806e]">Support workspace</p><h1 className="mt-1 text-3xl font-semibold tracking-[-.06em]">Good morning, {userName}.</h1><p className="mt-2 text-sm text-[#7d877c]">{published && sourceReady ? "Your support guide is live for customers." : sourceReady ? "Your knowledge is ready — publish when the answers feel right." : "Start by adding a knowledge source for your bot."}</p></div><button className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#202823] px-4 py-2.5 text-sm font-bold text-white transition hover:-translate-y-px hover:bg-[#354238]" onClick={() => onGoTo(sourceReady ? "widget" : "knowledge")} type="button">{published && sourceReady ? "Open live widget" : sourceReady ? "Publish widget" : "Add knowledge"} <ChevronRight size={15} /></button></div>
    {stats.sources === 0 && <OnboardingChecklist onGoTo={onGoTo} />}
    <div className="grid gap-3 sm:grid-cols-3">{metrics.map(([label, metricValue, metricNote, , Icon, progress], index) => <button aria-pressed={selectedMetric === index} className={`rounded-2xl border bg-white p-4 text-left transition hover:-translate-y-1 hover:shadow-lg ${selectedMetric === index ? "border-[#9ec47b] ring-2 ring-[#e3f1d6]" : "border-[#e0e6dc]"}`} key={label} onClick={() => setSelectedMetric(index)} type="button"><div className="flex justify-between"><p className="text-xs text-[#879087]">{label}</p><span className="grid h-8 w-8 place-items-center rounded-xl bg-[#eef5e8] text-[#6a953e]"><Icon size={15} /></span></div><p className="mt-4 text-2xl font-semibold tracking-[-.05em]">{metricValue || documents.length}</p><p className="mt-1 text-[11px] font-bold text-[#709744]">{metricNote}</p><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-[#edf2e9]"><div className="h-full rounded-full bg-[#88b95d] transition-[width]" style={{ width: `${progress}%` }} /></div></button>)}</div>
