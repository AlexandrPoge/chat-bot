...
export function ChatPanel({ activeSourceName, botName, isAnswering, messages, onAsk }: Props) {
  ...
  return (
    <section className="flex h-[min(520px,calc(100dvh-19rem))] min-h-[300px] flex-col overflow-hidden rounded-[1.25rem] border border-[#dfe7da] bg-white shadow-[0_22px_45px_-38px_rgba(32,45,31,.78)] sm:h-[min(580px,calc(100dvh-15rem))] sm:min-h-[380px] lg:h-[min(620px,calc(100dvh-10rem))] lg:min-h-[410px]">
      <header className="shrink-0 border-b border-[#e7ebe4] bg-white px-4 py-3.5 sm:px-5">
        <div className="flex flex-wrap items-center justify-between gap-3"><div className="min-w-0"><div className="flex items-center gap-2"><span className="grid h-7 w-7 place-items-center rounded-lg bg-[#edf7e6] text-[#5d9236]"><ShieldCheck size={14} /></span><h2 className="font-semibold tracking-[-.03em]">Customer-ready preview</h2></div><p className="mt-1 flex max-w-[260px] truncate text-xs text-[#71806e]"><FileText size={12} className="mr-1.5 shrink-0 text-[#6d9b40]" />Active source: {activeSourceName}</p></div><div className="flex w-fit shrink-0 rounded-lg bg-[#f1f5ee] p-1 text-[11px] font-bold"><button aria-pressed={mode === "test"} className={`rounded-md px-2.5 py-1.5 transition-all ${mode === "test" ? "bg-white text-[#4d7432] shadow-sm" : "text-[#899387]"}`} onClick={() => setMode("test")} type="button">✦ Test AI</button><button aria-pressed={mode === "live"} className={`rounded-md px-2.5 py-1.5 transition-all ${mode === "live" ? "bg-white text-[#4d7432] shadow-sm" : "text-[#899387]"}`} onClick={() => setMode("live")} type="button">Gemini AI</button></div></div>
      </header>
      <div aria-live="polite" className="min-h-0 flex-1 overflow-y-auto overscroll-contain bg-[#fbfcfa] p-3.5 [scrollbar-gutter:stable] sm:p-5" ref={listRef}><div className="space-y-3.5">{messages.map((message) => <div className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`} key={message.id}><article className={`max-w-[92%] break-words rounded-2xl px-3.5 py-3 text-sm leading-6 shadow-sm sm:max-w-[84%] sm:px-4 ${message.role === "user" ? "rounded-tr-md bg-[#202823] text-white" : "rounded-tl-md border border-[#e0e7dd] bg-white text-[#4d584d]"}`}><p>{message.content}</p>{message.followUp && <p className="mt-2 text-xs leading-5 text-[#6f7c6f]">{message.followUp}</p>}</article></div>)}{isAnswering && <div className="flex w-fit items-center gap-2 rounded-2xl rounded-tl-md border border-[#e1e7dd] bg-white px-4 py-3 text-sm text-[#839083]"><LoaderCircle className="animate-spin" size={15} />{botName} is thinking…</div>}</div></div>
