...
export function DashboardWorkspace() {
  ...
  const knowledge = useDashboardKnowledge({ notify, resetChat: chat.resetChat, saveSettings: writeBotSettings, settings });
  const embedCode = '<script async src="https://app.helpwise.ai/widget.js" data-bot="orbit_7Q92"></script>';
  const copyEmbedCode = async () => {
    await navigator.clipboard?.writeText(`<script async src="${window.location.origin}/widget.js" data-bot="orbit_7Q92"></script>`);
    setCopied(true); notify("Embed code copied"); window.setTimeout(() => setCopied(false), 1800);
  };
  const activatePlan = () => {
    if (choice === "Starter") { setPlan("Starter"); setBillingStep(null); notify("Starter remains active."); return; }
  ...
  const content = active === "conversations" ? <ConversationsPage activeDocument={knowledge.activeDocument} isAnswering={chat.isAnswering} messages={chat.messages} onAsk={(question, mode) => chat.askBot(question, mode, knowledge.activeDocument)} settings={settings} />
    ...
    : active === "widget" ? <WidgetPage activeDocument={knowledge.activeDocument} code={embedCode} copied={copied} onCopy={copyEmbedCode} />
      : active === "settings" ? <SettingsEditor documentCount={knowledge.documents.length} key={JSON.stringify(settings)} onReset={() => { writeBotSettings(DEFAULT_BOT_SETTINGS); notify("Default settings restored."); }} onSave={(value) => { writeBotSettings(value); notify("Settings saved and widget updated."); }} onUpgrade={() => setBillingStep("plans")} plan={plan} settings={settings} />
        : <OverviewPage activeDocument={knowledge.activeDocument} documents={knowledge.documents} isAnswering={chat.isAnswering} messages={chat.messages} onAsk={(question, mode) => chat.askBot(question, mode, knowledge.activeDocument)} onGoTo={setActive} settings={settings} userName={session?.email.split("@")[0] || ""} />;
  return <><DashboardShell active={active} onGoTo={setActive} onLogout={() => { clearDemoSession(); window.location.assign("/"); }} onProfileToggle={() => setProfileOpen((value) => !value)} onUpgrade={() => setBillingStep("plans")} plan={plan} profileOpen={profileOpen} session={session} toast={toast}>{content}</DashboardShell>{billingStep === "plans" && <PlanDialog choice={choice} onChoose={setChoice} onClose={() => setBillingStep(null)} onContinue={activatePlan} />}{billingStep === "checkout" && <TestCheckout onClose={() => setBillingStep("plans")} onPaid={() => { setPlan("Pro"); setBillingStep(null); notify("Pro is active. This was a test payment — no card was charged."); }} />}</>;
}
